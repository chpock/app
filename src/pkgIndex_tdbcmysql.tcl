...
package ifneeded tdbc::mysql 1.0.0 [list apply {{dir} {
  if { $::tcl_platform(os) eq "Windows NT" &&
    ...
    if { $_ eq "." } { error "couldn't find temp folder name for tdbc::mysql support library" }
    foreach fn [glob -types f -tails -directory $dir "*.dll"] {
      if { [string match -nocase "tdbcmysql*" $fn] } continue
      file copy -force [file join $dir $fn] [file join $_ $fn]
    }
  } {
    set _ [pwd]
  }
  source [file join $dir tdbcmysql.tcl]
  set tpwd [pwd]
  cd $_
  catch { load [file join $dir tdbcmysql100.dll] tdbcmysql } r o
  cd $tpwd
  return -options $o $r
}} $dir]
